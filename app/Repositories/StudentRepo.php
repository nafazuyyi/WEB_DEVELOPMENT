...
class StudentRepo {
    ...
    // 
    {
        return $this->activeStudents()->where(['my_class_id' => $class_id, 'session' => Qs::getCurrentSession()])->with(['my_class', 'user'])->get()->sortBy('user.name');
    }
    ...
    public function nonactiveStudents()
    // 
    {
        return StudentRecord::where(['grad' => 1]);
    }
    ...
    public function getRecordallGradStudents(array $data)
    // 
    {
        return $this->gradStudents()->where($data)->with('user');
    }

    // getRecordallGradStudents
    ...
    public function exists2($student_id)
    // 
    {
        return $this->nonactiveStudents(['user_id' => $student_id])->exists();
    }
