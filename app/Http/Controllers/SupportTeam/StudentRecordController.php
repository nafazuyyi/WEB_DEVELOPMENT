...
class StudentRecordController extends Controller
{
   ...
   public function __construct( MyClassRepo $my_class, UserRepo $user, StudentRepo $student)
   {
    ...
    public function showgradstudent($sr_id)
    {
        $sr_id = Qs::decodeHash($sr_id);
        // dd($sr_id);
        if(!$sr_id){return Qs::goWithDanger();}

        $data['sr'] = $this->student->getRecordallGradStudents(['id' => $sr_id])->first();
    ...
    public function update(StudentRecordUpdate $req, $sr_id)
    {
        ...
        $this->user->update($sr->user->id, $d); // Update User 

        $srec = $req->only(Qs::getStudentData());

        $this->student->updateRecord($sr_id, $srec); // Update Student
