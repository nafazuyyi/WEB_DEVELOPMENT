...
class MarkController extends Controller
{
    ...
    // 
    {
        ...
        foreach($marks->sortBy('user.name') as $mk)
        {
            ...
            $grade = $this->mark->getGrade($total);
            $d['grade_id'] = $grade ? $grade->id : NULL;
            $d['exm2'] = $exm2 = $mks['exm2_'.$mk->id];
    ...
    {
    // 
        $d = Qs::userIsTeamSAT() ? $req->only(['t_comment', 'p_comment','j_comment']) : $req->only(['t_comment']);
