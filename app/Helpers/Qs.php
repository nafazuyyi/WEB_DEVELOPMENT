...
class Qs
{
    ...
    {
        return self::getSetting('system_title') ?: 'Hubbul Khoir';
    }

    public static function getDefaultUserImage()
    {
        return app('url')->asset('global_assets/images/user.png');
    }

    public static function getDefaultSystemImage()
    {
        return app('url')->asset('global_assets/images/logo.png');
    }
    ...
    {
        $data = ['name', 'email', 'phone', 'phone2','dob', 'address','adm_no'];
    ...
    {
        $data = ['my_class_id','section_id', 'year_admitted', 'father', 'mother'];

        return $remove ? array_values(array_diff($data, $remove)) : $data;

    }

    public static function getStudentNote($remove = [])
    {
        $data = ['user_id','note'];
